...
public class Painting : MonoBehaviour
{
    ...
    {
        RenderTexture renderTexture = RenderTexture.GetTemporary(texRender.width, texRender.height, 0, RenderTextureFormat.ARGB32, RenderTextureReadWrite.Linear);
        ...
        Graphics.Blit(texRender, renderTexture);

        //修改RenderTexture的渲染纹理
        RenderTexture.active = renderTexture;
        //读取RenderTexture的渲染纹理像素信息，存储为纹理数据(Texture2D)
        texture2D.ReadPixels(new Rect(0, 0, texRender.width, texRender.height), 0, 0);

        byte[] bytes = texture2D.EncodeToPNG();
        if (!Directory.Exists(Application.dataPath))
            Directory.CreateDirectory(Application.dataPath + "/Resources");
        FileStream file = File.Open(Application.dataPath + "/Resources" + "/" + "Word" + Config.id + ".png", FileMode.Create);
